    ...
    <v-dialog v-model="dialog">
      ...
      <v-card>
        <v-card-text>
          ...
          <v-radio-group label="Label Type" v-model="mutable.labelColor">
          ...
          <v-layout row>
            <v-flex xs2>
              <v-select label="Select Icon" :items="icons" clearable autocomplete v-model="mutable.labelIcon"></v-select>
            </v-flex>
            <v-flex xs2>
              <v-btn v-if="mutable.labelIcon" flat><v-icon>{{mutable.labelIcon}}</v-icon></v-btn>
            </v-flex>
          </v-layout>
...
<script>
export default {
  props: ['label', 'color', 'icon', 'canEditField', 'width'],
  data () {
    return {
      mutable: {
        label: '',
